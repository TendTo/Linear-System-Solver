...
double *Gaussian_elimination_no_pivot_gpu_lmem(double *A, double *b, size_t n, cl_status *status)
{
    ...
    cl_mem d_U = clCreateBuffer(status->ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, U_memsize, h_U, &err);
...
double *Gaussian_elimination_no_pivot_gpu_buffer(double *A, double *b, size_t n, cl_status *status)
{
    ...
    //Create memory objects
    cl_mem d_even_U = clCreateBuffer(status->ctx, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, U_memsize, h_U, &err);
...
double *Gaussian_elimination_no_pivot_gpu_buffer_vec(double *A, double *b, size_t n, cl_status *status)
{
    ...
    //Create kernel
    cl_kernel gaussian_k = clCreateKernel(status->prog, "gaussian_elimination_no_pivot_buffer_vec", &err);
    ocl_check(err, "create gaussian_elimination_no_pivot_buffer_vec kernel");
    cl_kernel solve_k = clCreateKernel(status->prog, "gaussian_elimination_solve_buffer", &err);
    ocl_check(err, "create gaussian_elimination_solve_buffer kernel");
